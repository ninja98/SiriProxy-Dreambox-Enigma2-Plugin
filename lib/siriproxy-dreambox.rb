...
class SiriProxy::Plugin::Dreambox < SiriProxy::Plugin
  ...
  def get_live_schedule(fromdate, todate, onlylive=true, livenow=true, keyword="", listfuture=true)
  ...
  listen_for /([^ ]*) match.*of (.*) on TV( right now|today| tomorrow| this week|.*)/i   do |saylive,team,period|
    if saylive.strip.upcase == "LIVE" || saylive.strip.upcase == "LIFE"
     liveindicator = true
    end
    ...
    results = get_live_schedule(datefrom,dateto,liveindicator,nowonly,team, listfuture)
    matches = results[0]
    fails = results[1]
    if matches.size > 0
      count = 1
  ...
  listen_for /([^ ]*) match.*TV( right now| today| tomorrow| this week).*/i do |saylive, period|
    puts "Period #{period}"
    datefrom = Time.now
    dateto = Time.now + (3600*24*7)
    liveindicator = false
    #liveindicator =  true if live.match(/live$/i) 
    listfuture = true
    if saylive.strip.upcase == "LIVE" || saylive.strip.upcase == "LIFE"
     liveindicator = true
    end
    ...
    results = get_live_schedule(datefrom,dateto,liveindicator, true, "" , listfuture)
